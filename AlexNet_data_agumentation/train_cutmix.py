...
from utils.data import cutmix_data,cutmix_loss
...
def main():
    ...
    # 创建tensorboard实例
    tb = SummaryWriter(log_dir="./tensorboard/AlexNet-cutmix1.5")
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            images, labels = data
            # 清空梯度
            cm_images, cm_labels, lam = cutmix_data(images, labels, 1.5)
            optimizer.zero_grad()
            outputs = net(cm_images.to(device))
            loss = cutmix_loss(loss_function, outputs.to(device), cm_labels.to(device), lam)
            loss.backward()
            # 更新学习参数
            optimizer.step()
        ...
        tb.add_scalars("AlexNet", {'train_loss_cutmix': train_loss, 'val_accuracy_cutmix':  val_acc}, epoch)
